...
- Generate a high-quality, photorealistic result`;
    ...
    console.log("Sending request to Gemini AI for image transformation...");
    ...
    console.log("Gemini AI image transformation completed:", generatedImageUrl);
    ...
    return NextResponse.json({
      ...
      promptId: promptId,
      textResponse: textResponse || "Image transformation generated successfully"
    });
    ...
    if (error.message?.includes("safety filters") || error.message?.includes("blocked")) {
      return NextResponse.json({ 
        error: "Content was blocked by safety filters. Please try with different images or ensure appropriate content.",
        type: "SAFETY_FILTER",
        provider: "gemini",
        details: "The AI model's safety systems prevented generation. Try with different images."
      }, { status: 400 });
    }
    
    return NextResponse.json({ 
      error: error instanceof Error ? error.message : "Failed to process image transformation request",
      provider: "gemini-prompt"
    }, { status: 500 });
  }
}
